 ...
 * desc:加载等待的Dialog
 */
public class LoadingTipDialog implements FinishDrawListener {
    public final String TAG = "LoadingTipDialog";
    ...
    public void dispatchFinishEvent(View v) {
        if (v instanceof WrongDiaView) {
            handler.sendEmptyMessageDelayed(HANDLE_WHAT_WRONG, time);
        } else {
            handler.sendEmptyMessageDelayed(HANDLE_WHAT_RIGHT, time);
        }
    }
    ...
    private final static int HANDLE_WHAT_RIGHT = 1;
    private final static int HANDLE_WHAT_WRONG = 2;
    ...
    private Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            LoadingTipDialog.this.close();
            switch (msg.what) {
                case HANDLE_WHAT_RIGHT:
                    if (onFinishListener != null)
                        onFinishListener.onFinish(FinishResult.RIGHT);
                    break;
                case HANDLE_WHAT_WRONG:
                    if (onFinishListener != null)
                        onFinishListener.onFinish(FinishResult.WRONG);
                    break;
            }
        }
    };
     ...
     * @param style
     */
    public LoadingTipDialog setLoadStyle(int style) {
        if (style >= 3) {
     ...
     * @return 这个对象
     */
    public LoadingTipDialog setLoadingText(String msg) {
     ...
     * @return 这个对象
     */
    public LoadingTipDialog setSuccessText(String msg) {
        if (msg != null && msg.length() > 0)
            loadSuccessStr = msg;
        return this;
    }
     ...
     * @return 这个对象
     */
    public LoadingTipDialog setFailedText(String msg) {
     ...
     * 关闭动态绘制
     */
    public LoadingTipDialog closeSuccessAnim() {
     ...
     * 关闭动态绘制
     */
    public LoadingTipDialog closeFailedAnim() {
     ...
     * @return 这个对象
     */
    public LoadingTipDialog setInterceptBack(boolean interceptBack) {
     ...
     * @return 这个对象
     */
    public LoadingTipDialog setLoadSpeed(Speed speed) {
     ...
     */
    private LoadingTipDialog setDrawColor(@ColorInt int color) {
     ...
     * @return 这个对象
     */
    public LoadingTipDialog setSize(int size) {
//        int dip = SizeUtils.px2dip(mContext, size);
     ...
     * @return 这个对象
     */
    public LoadingTipDialog setRepeatCount(int count) {
     ...
     * @return 这个对象
     */
    public LoadingTipDialog setShowTime(long time) {
     ...
     * @return 这个对象
     */
    public LoadingTipDialog setTextSize(float size) {
     ...
     * 传递绘制完成的事件
     *
     * @param onFinishListener 回调接口
     */
    public LoadingTipDialog setOnFinishListener(OnFinishListener onFinishListener) {
